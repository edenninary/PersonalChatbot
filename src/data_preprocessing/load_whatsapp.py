...
def preprocess_convo(input_path, output_path, role="person2", chat_owner="Eden Ninary"):
    ...
    df["chat_message"] = df.apply(lambda x: ("person1" if x["username"] == chat_owner else role, x["message"]), axis=1)
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        chat_message_formatted = "<s><{role}>{message}".format(role=row_role, message=row_message)
    ...
    df_model['query_str'] = df_model['query'].apply(lambda x: "</s>".join(x))
